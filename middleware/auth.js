...
function authenticate(req, res, next) {
  ...
  const query = "SELECT id, faculty_id, department, email, name, status FROM users WHERE id = ?";
...
function ensureFacultyCanManageLab(req, res, next) {
  ...
  const query = "SELECT 1 FROM faculty_labs WHERE faculty_id = ? AND lab_name = ? LIMIT 1";
